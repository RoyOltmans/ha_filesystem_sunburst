...
Save the `filesystem_scanner.py` script to a directory on your server.

Place the script in a secure location, such as `/opt/scripts/filesystem_scanner.py`. Run the script using the following command:
```bash
